...
public class JobSchedulingController {
    ...
    @CrossOrigin(origins = "*")
    @SecurityRequirement(name = "Bearer")
    @Operation(summary = "Listar Serviços de um agendamento", description = "Retorna uma lista de todos os serviços de um agendamento")
    @ApiResponse(responseCode = "200", description = "Lista de Serviços retornada com sucesso")
    @ApiResponse(responseCode = "204", description = "Lista de Serviços retornada com sucesso mas vazia")
    @ApiResponse(responseCode = "404", description = "Agendamento não encontrado")
    @GetMapping("/{idScheduling}/servicos")
    public ResponseEntity<List<Job>> getJobsById(@PathVariable Integer idScheduling){
        List<Job> jobsInScheduling = jobSchedulingService.getJobsInScheduling(idScheduling);
